...
- Classification: **READ-ONLY**
- Expected output: bounded local parsing reports four exact DRAFT legal identities and both direct contact links on both landings
...
Record the six response hashes, exact markers, origin, release SHA, operator, and UTC time. Do not store the HTML bodies in the evidence packet.
...
Record HTTPS/TLS evidence, route smoke, six response hashes, exact release/legal/contact markers, disabled form state, and rejected API routes. Only then hand off to `form-activation.md`. DNS approval does not authorize form activation.
